...
export default function ContentManager() {
    ...
    const [editingService, setEditingService] = useState(null); // New state for editing service
    ...
    const handleSaveSettings = async () => {
        ...
        try {
            ...
            if (settings.about_extra_image_file) formData.append('about_extra_image', settings.about_extra_image_file);
    ...
    return (
        <div className="p-4 md:p-8 pt-20 lg:pt-8 space-y-10 max-w-7xl mx-auto">
                    ...
                    {services.length > 0 ? services.map(service => (
                        <div key={service.id} className="bg-white p-6 rounded-[2rem] border border-gray-100 flex items-center justify-between group hover:border-green-200 transition-all shadow-sm">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => openEditService(service)} className="p-2 text-gray-300 hover:text-green-600 transition-colors">
                                    <Edit3 size={20}/>
                                </button>
            ...
            {/* Rest of your UI (About, Partners, Portfolio) remains exactly the same */}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    {partners.map(partner => (
                        <div key={partner.id} className="bg-white p-4 rounded-2xl border border-gray-100 relative group aspect-square flex items-center justify-center">
                            <img src={`${BASE_URL}/storage/${partner.path}`} className="max-h-full max-w-full object-contain grayscale group-hover:grayscale-0 transition-all" alt="Partner"/>
                            <button onClick={() => handleDeletePartner(partner.id)} className="absolute top-2 right-2 p-1.5 bg-red-50 text-red-500 rounded-lg opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500 hover:text-white"><Trash2 size={14}/></button>
                        </div>
                    ))}
                </div>
            </section>
            ...
            <section className="space-y-6 pb-20">
                 <div className="flex justify-between items-center px-2">
                    ...
                    <button onClick={openAddPortfolio} className="bg-green-600 text-white p-3 rounded-xl hover:shadow-lg transition-all hover:scale-110 active:scale-95"><Plus size={20}/></button>
                </div>
                    ...
                    {portfolio.map(item => (
                        <div key={item.id} className="bg-white p-4 rounded-[2rem] border border-gray-100 relative group overflow-hidden shadow-sm hover:shadow-md transition-all">
                            <div className="aspect-video w-full overflow-hidden rounded-2xl mb-4 bg-gray-100">
                                <img src={`${BASE_URL}/storage/${item.image_path}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt={item.title}/>
                            </div>
                            <div className="px-2 flex justify-between items-start">
                                ...
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                    <button onClick={() => openEditPortfolio(item)} className="p-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-green-600 hover:text-white transition-colors"><Edit3 size={16}/></button>
                                    <button onClick={() => handleDeletePortfolio(item.id)} className="p-2 bg-gray-100 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-colors"><Trash2 size={16}/></button>
                                </div>
                            </div>
                        </div>
                    ))}
